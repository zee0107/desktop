...
function Run-BuildForceSignature {
    ...
    if (Test-Path 'env:PFX') {
        ...
        foreach ($archPath in "release\win-unpacked", "release\win-ia32-unpacked") {
            ...
            Print-Info "Signing Crypter.exe (waiting for 2 * 15 seconds)..."
            Start-Sleep -s 15
            signtool.exe sign /f "./mattermost-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha1 /td sha1 "$archPath\Crypter.exe"
            Start-Sleep -s 15
            signtool.exe sign /f "./mattermost-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha256 /td sha256 /as "$archPath\Crypter.exe"
        }
    } else {
